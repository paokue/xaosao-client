...
import { deleteServiceBooking } from "~/services/booking.server";
...
export async function action({ params, request }: ActionFunctionArgs) {
   ...
   if (request.method === "DELETE") {
      try {
         const res = await deleteServiceBooking(id!, customerId);
         if (res.id) {
            return redirect(`/customer/dates-history?toastMessage=Delete+date+booking+successfully!&toastType=success`);
         }
      } catch (error: any) {
         if (error?.payload) {
